    ...
    private suspend fun syncChannel(channel: ChannelPreferences.SavedChannel) {
        try {
            ...
            if (violations.isNotEmpty()) {
                ...
                // FILTER OUT ALREADY FIRED ALERTS AND CONTINUOUS VIOLATIONS (EDGE-TRIGGERED)
                val newViolations = violations.filter { violation ->
                    val fired = firedAlertDao.getFiredAlert(channel.id, violation.fieldNumber)
                    
                    if (previousEntry != null) {
                        // Check if previous entry also violated
                        val wasViolatingLegacy = channelAlerts.any { it.fieldNumber == violation.fieldNumber } &&
                                checkAlerts(previousEntry, channelAlerts.filter { it.fieldNumber == violation.fieldNumber }).isNotEmpty()
                        
                        val prevFieldValue = previousEntry.fields[violation.fieldNumber]?.toDoubleOrNull()
                        val wasViolatingAdvanced = advancedRules.any { rule -> 
                            rule.fieldNumber == violation.fieldNumber && prevFieldValue != null && when (rule.condition) {
                                "GREATER_THAN" -> prevFieldValue > rule.thresholdValue
                                "LESS_THAN" -> prevFieldValue < rule.thresholdValue
                                else -> false
                            }
                        }
                        
                        val wasViolating = wasViolatingLegacy || wasViolatingAdvanced
                        
                        // Only trigger if state changed from non-violating to violating
                        !wasViolating && (fired == null || fired.lastFiredEntryId < latestEntry.entryId)
                    } else {
                        // Fallback if no history
                        fired == null || fired.lastFiredEntryId < latestEntry.entryId
                    }
                }
